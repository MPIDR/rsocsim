...
THIS implements socsimulation for the Global Sandwcih project
wherein all the countries of the world are simulated and we measure
the degree of sandwichness across all of them.

since we lack marriage rates -- or rather the UN lacks such rates,
we will implemetn the following wheeze:

whenever a woman has a birth event, we will check if the woman is
single. if so, we will execute a marriage before we create the child.
marriage will be by the one queue system -- or the 2q with very high
rates for males.  i.e. when we tell her get married; she will do it. 
suitors shall be chosen by distribution
     ...
     **/
{
  ...
  sprintf(logstring,
	  "\n%s: read_xtra_file=%d\n",ENHANCEMENT_NAME,read_xtra_file);
  logmsg(logstring,logstring,1);

  sprintf(logstring,
	  "%s: size_of_xtra is %d\n",ENHANCEMENT_NAME,size_of_extra);
  logmsg(logstring,logstring,1);
...
/**************************************************************************/
void enhance_birth_pre(p)
  struct person *p;
  {
    /* called from events.c process month INSTEAD of birth(p) when
       ENHNACED is defined.  Consequently, the noop of this is to simply
       leave the call to birth(p) alone.
     ...
     struct person *p, *spouse;
{
  /** This gets called after a successful marriage -- that is at the
      ...
      has been created and filled in  called INSTEAD of new_marriage()
  **/
